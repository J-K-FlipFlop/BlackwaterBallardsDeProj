...
class TestConvertCounterParty:
    def test_convert_counter_rtns_df_type_removes_drop_cols_and_adds_loc_cols(
        self, s3_client, file_name1="address", file_name2="counterparty"
    ):
    ...
    def test_convert_counter_rtns_expected_data_from_dataframe(
        self, s3_client, file_name1="address", file_name2="counterparty"
    ):
        timestamp = "2024-05-20 12:10:03.998128"
        filename1 = f"test/data/{file_name1}.csv"
        ...
        assert list(result["data"]["counterparty_legal_name"][0:10]) == [
            "Fahey and Sons",
        ...
        assert list(
            result["data"]["counterparty_legal_address_line_1"][0:10]
        ) == [
        ...
        assert list(
            result["data"]["counterparty_legal_address_line_2"][0:10]
        ) == [
            "Axel Freeway",
            nan,
            nan,
        ...
        assert list(result["data"]["counterparty_legal_district"][0:10]) == [
            nan,
            nan,
            nan,
            nan,
        ...
        assert list(result["data"]["counterparty_legal_city"][0:10]) == [
            "East Bobbie",
            "Utica",
            "Aliso Viejo",
            "Bartellview",
        ...
        assert list(
            result["data"]["counterparty_legal_postal_code"][0:10]
        ) == [
            "88253-4257",
            "93045",
            "99305-7380",
        ...
        assert list(result["data"]["counterparty_legal_country"][0:10]) == [
        ...
        assert list(
            result["data"]["counterparty_legal_phone_number"][0:10]
        ) == [
            "9687 937447",
            "7772 084705",
            "9621 880720",
    ...
    def test_convert_counterparty_without_req_file_returns_expected_error(
        self, s3_client, file_name1="address", file_name2="department"
    ):
        timestamp = "2024-05-20 12:10:03.998128"
        filename1 = f"test/data/{file_name1}.csv"
        filename2 = f"test/data/{file_name2}.csv"
        ...
        assert (
            str(result["message"])
            == f"No files Found on: s3://{bucket}/ingested_data/{timestamp}/counterparty.csv."
        )

    def test_convert_counterparty_without_req_bucket_returns_expected_error(
        self, s3_client, file_name1="address", file_name2="counterparty"
    ):
        timestamp = "2024-05-20 12:10:03.998128"
        filename1 = f"test/data/{file_name1}.csv"
