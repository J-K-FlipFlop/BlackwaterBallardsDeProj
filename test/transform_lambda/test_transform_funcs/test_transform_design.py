...
class TestConvertDesign:
    def test_convert_design_rtns_df_type_and_removes_drop_cols(
    ...
    def test_convert_design_rtns_expected_data_from_dataframe(
        self, s3_client, file_name="design"
    ):
        timestamp = "2024-05-20 12:10:03.998128"
        filename = f"test/data/{file_name}.csv"
        ...
        assert list(result["data"]["design_name"][0:10]) == [
            "Wooden",
            "Bronze",
            "Granite",
            "Bronze",
        ...
        assert list(result["data"]["file_location"][0:10]) == [
            "/usr",
            "/private",
            "/private/var",
            "/lost+found",
        ...
        assert list(result["data"]["file_name"][0:10]) == [
            ...
            "concrete-20210614-04nd.json",
            "soft-20220201-hzz1.json",
            "cotton-20220527-vn4b.json",
            "granite-20220125-ifwa.json",
        ]

    def test_convert_design_without_req_file_returns_expected_error(
        ...
        assert (
            str(result["message"])
            == f"No files Found on: s3://{bucket}/ingested_data/{timestamp}/design.csv."
        )

    def test_convert_design_without_req_bucket_returns_expected_error(
        self, s3_client, file_name="design"
    ):
        timestamp = "2024-05-20 12:10:03.998128"
        filename = f"test/data/{file_name}.csv"
