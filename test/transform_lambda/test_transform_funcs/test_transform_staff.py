...
class TestConvertStaff:
    def test_convert_staff_rtns_df_type_removes_drop_cols_and_adds_dept_cols(
        self,
        s3_client,
        file_name1="staff",
        file_name2="department",
        file_name3="last_ran_at",
    ):
    ...
    def test_convert_staff_rtns_expected_data_from_dataframe(
        self, s3_client, file_name1="staff", file_name2="department"
    ):
        timestamp = "2024-05-20 12:10:03.998128"
        filename1 = f"test/data/{file_name1}.csv"
        key1 = f"ingested_data/{timestamp}/{file_name1}.csv"
        ...
        result = convert_staff(s3_client, session)
        assert list(result["data"]["staff_id"][0:10]) == [
            1,
            2,
            3,
            4,
        ...
        assert list(result["data"]["first_name"][0:10]) == [
            "Jeremie",
            "Deron",
            "Jeanette",
            "Ana",
        ...
        assert list(result["data"]["last_name"][0:10]) == [
            "Franey",
            "Beier",
            "Erdman",
            "Glover",
        ...
        assert list(result["data"]["department_name"][0:10]) == [
            "Purchasing",
            "Facilities",
            "Facilities",
            "Production",
        ...
        assert list(result["data"]["location"][0:10]) == [
            "Manchester",
            "Manchester",
            "Manchester",
            "Leeds",
        ...
        assert list(result["data"]["email_address"][0:10]) == [
    ...
    def test_convert_staff_without_req_file_returns_expected_error(
        self, s3_client, file_name1="design", file_name2="department"
    ):
        timestamp = "2024-05-20 12:10:03.998128"
        filename1 = f"test/data/{file_name1}.csv"
        filename2 = f"test/data/{file_name2}.csv"
        ...
        assert (
            str(result["message"])
            == f"No files Found on: s3://{bucket}/ingested_data/{timestamp}/staff.csv."
        )

    def test_convert_staff_without_req_bucket_returns_expected_error(
        self, s3_client, file_name1="staff", file_name2="department"
    ):
        timestamp = "2024-05-20 12:10:03.998128"
        filename1 = f"test/data/{file_name1}.csv"
